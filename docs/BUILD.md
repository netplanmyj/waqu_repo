...
この手順書では、水質検査報告アプリのAndroid APKファイルをビルドする方法を説明します。
...
#### 5. GitHub CLIでの文字化け
macOSでGitHub CLI (`gh pr create`) を使用する際、バックスラッシュ (`\`) やバッククォート (`` ` ``) を含むメッセージは文字化けします。

**回避方法:**
- GitHub Web UIから直接PRを作成
- または、これらの文字を使用せずにメッセージを記述
- ファイルからメッセージを読み込む: `gh pr create --body-file description.md`

```bash
# 推奨: 説明ファイルを使用
cat > pr_description.md << 'EOF'
## 概要
Firebase認証のエラーに対する安定性を向上させました。
EOF

gh pr create --title "Fix: ネットワークエラーの安定性向上" --body-file pr_description.md
```
...
これで水質検査報告アプリの配布用APKファイルが準備できました。